...
\frame{
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Access widget value in R object with the widget's label
		\end{itemize}

		\item R object (eg. plot) $\rightarrow$ cf. tutorial/lesson4
		\begin{itemize}
			\item \texttt{*Output} functions create R objects
			\item The arg of \texttt{*Output} has to match R object's label in \texttt{server.R}
		\end{itemize}
...
\frame{
	...
	\begin{itemize}
		\item The output `list' stores R objects
			\begin{itemize}
				\item \texttt{render*} function: instructions to create object $\rightarrow$ tutorial/lesson4
		...
		\begin{itemize}
			\item Call a widget value inside a \texttt{render*} function using the label of the widget
		\end{itemize}
